...
@MainActor
@Suite("AppFeature — 강제 업데이트")
struct AppFeatureForceUpdateTests {
    ...
    @Test("버전 체크를 통과하면 세션 복원·프로필 조회로 이어진다")
    ...
    func blocksOnForcedUpdateWithoutFetchingProfile() async {
        // restoreSessionUseCase·fetchMyProfileUseCase는 일부러 주입하지 않는다 — 호출되면 unimplemented로 실패한다.
        let store = TestStore(initialState: AppFeature.State.launching) {
            AppFeature()
        } withDependencies: {
