...
| `CHALLATheme` + `EnvironmentValues.challaTheme` | 사용자가 고른 테마의 색 묶음 — `accent`(강조 요소 공통) + `glow`(`challaMainBackground()` 전용 번짐). 앱 루트가 `\.challaTheme`에 한 번 주입하면 하위 전체가 따른다. 주입이 없으면 `lemonade`라 프리뷰·검수앱·데모앱은 배선 없이 그려진다. `glow`를 생략한 init은 `accent`로 떨어진다. 앱은 테마와 무관하게 브랜드 색 하나를 번짐에 쓴다 |
