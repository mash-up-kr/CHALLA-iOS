...
private enum ListSectionMetric {
    ...
    /// 카드 안쪽 오른쪽 여백.
    ///
    /// 왼쪽(24)보다 작지만 오타가 아니다 — `List / Arrow` 컴포넌트 폭이 318로 고정이고
    /// 카드 폭이 358이라 24 + 318 + 16 으로 딱 맞아떨어진다.
    ///
    /// 체크·토글 행의 안여백은 20이다 — 컴포넌트 정의(`List / Check`)는 21이지만
    /// 화면 인스턴스(테마·알림 화면)가 20이고, 배포되는 건 화면이라 화면을 따른다.
    /// 행 종류마다 카드를 갈아끼울 수 없으므로 그 4는 행 쪽에서 더한다
    /// (`ListRowMetric.accessoryTrailingInset`).
