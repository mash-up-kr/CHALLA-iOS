...
private enum ListSectionMetric {
    ...
    /// 헤더 블록은 높이 44에 글자가 위에서 16 내려온 자리에 있다 (아래 12).
    /// `challaFont`가 글자 상자 위아래에 각각 여백을 더하므로 그만큼 뺀다.
    static let headerTopPadding: CGFloat = max(0, 16 - ListSectionMetric.headerTypography.lineBoxInset)

    static let headerBottomPadding: CGFloat = max(0, 12 - ListSectionMetric.headerTypography.lineBoxInset)
}
