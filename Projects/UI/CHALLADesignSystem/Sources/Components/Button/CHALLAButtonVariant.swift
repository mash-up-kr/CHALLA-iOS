...
public enum CHALLAButtonVariant: Sendable, CaseIterable {
    ...
    /// 테마 색을 쓰는 유일한 variant다. 테마 시스템이 없어 지금은 `defaultTheme` 고정이고,
    /// 호출부가 색을 넘기는 통로는 #35(테마 선택)가 오면 `CHALLAListRow`처럼 연다.
...
extension CHALLAButtonVariant {
    ...
    func backgroundColor(role: CHALLAButtonRole? = nil, isEnabled: Bool) -> Color? {
        ...
        case (.theme, true): return CHALLAColor.defaultTheme
        case (.primary, true):
