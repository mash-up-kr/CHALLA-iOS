...
방 목록·필터 목록·필터 LUT·카메라 권한을 먼저 갖추고, 전부 성공했을 때만 넘어간다.
...
- `enum ShootPreparationError` — 준비 실패
  (`.cameraPermissionDenied` · `.loadFailed(message:)`)
...
- 묻는 권한은 **카메라뿐이다** — 촬영본은 사진첩에 저장하지 않고 업로드로만 이어진다
...
- **이 모듈이 의존**: `RoomDomain` · `PhotoDomain` · `ComposableArchitecture`(AlertState)
...
화면이 없는 순수 로직이라 시뮬레이터 없이 돈다. 성공 결과, 카메라 권한 거절,
조회·LUT 실패 문구, 권한 거절 우선순위를 검증한다.
