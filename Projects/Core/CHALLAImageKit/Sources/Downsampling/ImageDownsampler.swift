...
///
/// `UIImage(data:)`로 원본 전체를 디코딩하면 4000×3000 사진 한 장이 약 45MB
/// (4000×3000픽셀 × 4바이트) 메모리를 차지한다 — 파일 크기(수 MB)와 무관하다.
/// 이 타입은 ImageIO의 썸네일 디코딩(`CGImageSourceCreateThumbnailAtIndex`)으로
...
public struct ImageDownsampler: Sendable {
    ...
    /// - `kCGImageSourceThumbnailMaxPixelSize` — 업스케일링 방지
    ///
