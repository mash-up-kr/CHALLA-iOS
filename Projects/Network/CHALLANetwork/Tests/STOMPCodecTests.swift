...
struct STOMPCodecTests {
    ...
    private func decode(_ text: String) throws -> (frames: [STOMPFrame], remainder: Data) {
        try STOMPCodec.decode(Data(text.utf8))
    }
    ...
    func invalidUTF8() {
        ...
        data.append(Data("\n\n\0".utf8))
        #expect(throws: STOMPError.self) { try STOMPCodec.decode(data) }
    }

    @Test("':'가 없는 헤더 줄은 malformedFrame으로 알린다")
    func headerWithoutSeparator() {
        #expect(throws: STOMPError.self) { try STOMPCodec.decode(Data("MESSAGE\nbroken\n\n\0".utf8)) }
    }
