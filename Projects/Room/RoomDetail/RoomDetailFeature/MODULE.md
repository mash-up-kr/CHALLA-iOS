...
버튼을 누르면 `ShootEntry`의 `ShootPreparation`이 촬영 가능 방 목록·필터(LUT 포함)·카메라 권한을
...
- **당길 곳을 알리는 튕김은 툴팁만 받는다.** 필름까지 움직이면 출구에서 밀려 나왔다
  들어가는 것처럼 보여 무겁다
- 튕김은 놓을 때 감쇠를 낮춰(bounce 0.7) 제자리를 여러 번 넘나들며 잦아든다.
  감쇠를 높이면 튕김이 아니라 밀림으로 보인다
