...
/// 인화 완료 안내 기록을 기기에 담아두는 키-값 저장소 추상.
///
/// `UserDefaults`를 직접 쓰지 않고 한 겹 두는 이유는 테스트 때문이다 —
/// 실제 `UserDefaults`를 쓰면 테스트끼리 상태가 새고 실행 순서에 결과가 흔들린다.
/// (`PhotoData.CameraOnboardingStorage`와 같은 판단이지만, Data 모듈끼리 import 하지 않으려고 따로 둔다.)
public protocol PrintNoticeStorage: Sendable {
...
public struct UserDefaultsPrintNoticeStorage: PrintNoticeStorage, @unchecked Sendable {
