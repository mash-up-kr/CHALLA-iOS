...
2. Feature는 Data를 import 하지 않는다 — DIContainer가 런타임에 구현체를 주입한다.
...
- Feature에서 Data의 타입/API가 필요하다 → Domain에 인터페이스(UseCase·Repository)를 추가하고 Feature는 그것만 사용. 구현은 Data에, 연결은 DIContainer에.
